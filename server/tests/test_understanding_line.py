...
class UnderstandingLineTest(unittest.TestCase):
    ...
    def test_有理解_摘要按记录顺序用分号连_把握取最低一档_高时不括注(self):
        root = self.dir([
            recorded("u-1",
                     {"act_id": "r1-1", "summary": "同意 UC-001、UC-002", "confidence": "high"},
                     {"act_id": "r1-2", "summary": "纠正 UC-003 的参与者", "confidence": "high"},
                     {"act_id": "r1-3", "summary": "TBD-002 先不管", "confidence": "medium"}),
            recorded("u-2", {"act_id": "r2-1", "summary": "把材料整理成条目", "confidence": "high"}),
            # 早先记下的事件内容里没有把握：到对话行为表里按编号查。
            recorded("u-3", {"act_id": "r3-1", "summary": "问保留几天"}),
        ], acts=[("r3-1", "low")])
        lines = work_summary.understanding_lines(root, SESSION)
        self.assertEqual(lines["u-1"], "理解为：同意 UC-001、UC-002；纠正 UC-003 的参与者；TBD-002 先不管（把握中）")
        self.assertEqual(lines["u-2"], "理解为：把材料整理成条目")
        self.assertEqual(lines["u-3"], "理解为：问保留几天（把握低）")

    def test_只记了无效理解时写正在重写_之后写对了就换成理解(self):
        root = self.dir([invalid("u-1"), invalid("u-2"), recorded("u-2", {"act_id": "r2-1", "summary": "寒暄", "confidence": "high"})])
        lines = work_summary.understanding_lines(root, SESSION)
        self.assertEqual(lines["u-1"], "助手的理解没有按格式写，正在重写")
        self.assertEqual(lines["u-2"], "理解为：寒暄")
    ...
    def test_刷新后的对话记录里过程摘要带理解(self):
        root = self.dir([recorded("u-1", {"act_id": "r1-1", "summary": "把材料整理成条目", "confidence": "high"})])
        entries = [
        ...
        self.assertEqual(summary["understanding"], "理解为：把材料整理成条目")
    ...
    def test_实时推送_助手消息落进会话时推一行理解_排在步骤最前_写对了用同一个键换掉(self):
        ...
        conn.execute("INSERT INTO event VALUES (2, ?, 'USER_INTENT_RECORDED', ?)",
                     (SESSION, json.dumps({"user_entry": "u-1", "origin": "understanding",
                                           "acts": [{"act_id": "r1-1", "summary": "整理材料", "confidence": "high"}]}, ensure_ascii=False)))
        conn.commit()
        conn.close()
        ex._understanding_step(SESSION)
        ...
        self.assertEqual((steps[-1]["step_key"], steps[-1]["text"], steps[-1]["in_progress"]), ("w-u-1-intent", "理解为：整理材料", False))
