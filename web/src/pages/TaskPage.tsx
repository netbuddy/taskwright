...
import { completionHeadline, confirmState, reviewState } from "../model/items";
...
export function TaskPage({ taskId }: { taskId: string }) {
  ...
  return (
    <Shell currentTaskId={taskId}>
      <div className="muted small" style={{ marginBottom: 6 }}><a href={href.tasks()}>任务</a> › {task.task_name}</div>
      ...
      {closed && <Alert type="info" showIcon style={{ marginBottom: 12 }} message={`这个任务${task.status}，整页只读：不能新建会话、上传材料或修改条目，生成文档照常可用。`} />}

      <div className="section-title">交付物看板 <span className="muted" style={{ fontWeight: 400 }}>按条目集合分开看：每个集合现在有几个条目、最新的一版是第几版、有几个条目的当前版本已经评审通过、有几个已经经过用户确认。</span></div>
      <div className="board">
        {task.definition.collections.map((coll) => {
          ...
          const confirmed = items.filter((i) => confirmState(i) === "confirmed").length;
          const latest = items.reduce((m, i) => Math.max(m, i.version_no), 0);
          return (
            <div className="card" key={coll.name}>
              ...
              <div className="chips">
                {items.length > 0 && <span className="chip">最新版本：第 {latest} 版</span>}
                <span className={`chip ${reviewed === items.length && items.length ? "ok" : "warn"}`}>评审通过 {reviewed}/{items.length}</span>
                <span className={`chip ${confirmed === items.length && items.length ? "ok" : "warn"}`}>用户确认 {confirmed}/{items.length}</span>
              </div>
              <div className="muted small">
                {items.length === 0 ? "这个集合还没有条目。" : `这 ${items.length} 个条目里，${reviewed} 个的当前版本有评审通过的记录，${confirmed} 个的当前版本有用户接受的记录。`}
              </div>
            </div>
          );
        })}
      ...
      <div className="two-col">
        ...
        <div>
          ...
          <div className="card">
            <div className="small" style={{ marginBottom: 8 }}>这个任务现在有 {task.materials.length} 份材料。执行者读的就是这几份文件。</div>
            {task.materials.map((m) => (
              <div key={m.path} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0" }}>
            ...
            {!closed && (
              ...
              >
                <span className="muted small">把文件拖到这里，或者点这里选择文件（只收 .md 与 .txt，单个不超过 5 MB）。新传的材料下一次会话开始时执行者就能看到。</span>
              </Upload.Dragger>
            )}
          </div>
          ...
          <div className="card">
            ...
            {task.sessions.map((s) => (
              <div key={s.session_id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 0" }}>
      ...
      <DocumentModal task={task} open={docOpen} onClose={() => setDocOpen(false)} />
      <Modal title={material?.path} open={!!material} onCancel={() => setMaterial(null)} footer={null} width={760}>
        <pre className="doc-text" style={{ maxHeight: 560, overflow: "auto" }}>{material?.text}</pre>
      </Modal>
    </Shell>
  );
}
