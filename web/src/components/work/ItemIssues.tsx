...
//     输入框为空时只预填对话区输入框、不发送；「先不管，保留」走 keep_pending（notify_executor: false）。
...
// 灰化与其它写入按钮一致（任务已结束、助手不可用、助手工作中、正在保存），悬停说明原因；
// 有未保存的条目编辑时「回答」也灰化，因为它要发到对话区，与对话区的发送键同一条规矩。
//
// 卡片上各字段按任务定义的顺序认，不写死字段名：第一个字段是事项；状态字段（带「用户决定保留」取值的枚举）之前的文本字段
// 是助手写的建议，之后的文本字段是了结时写的处理结果；状态以外的枚举字段（例如种类）作小标签。
...
function IssueCard({ task, issue, hi, readOnly, writesOff, hold, pending, submit, onSend, onPrefill }: {
  ...
  const statusAt = others.findIndex((f) => f.name === statusField.name);
  const filled = (f: (typeof others)[number]) => f.type === "文本" && !isEmptyValue(issue.fields[f.name]);
  const suggestions = others.slice(0, statusAt).filter(filled);
  const outcomes = others.slice(statusAt + 1).filter(filled);
  ...
  const answerOff = keepOff || hold;
  const answerTitle = keepTitle ?? (hold ? HOLD_TEXT : undefined);
    ...
    <div className={`sw-iss-card${open ? "" : " closed"}${hi ? " hi" : ""}`} data-testid={`issue-card-${issue.item_id}`}>
      ...
      {open && (
        <div className="ans">
          ...
          <button type="button" className="btn sm" disabled={answerOff} title={answerTitle ?? "发给助手；输入框空着时只把开头填进对话区输入框"} onClick={doAnswer}
