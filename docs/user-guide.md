...
The task page shows the task's status, its completion conditions, its materials and its sessions. Upload the materials in the **Materials** (材料清单) box: drag files onto it or click it to choose them. Plain-text `.md` or `.txt` files and Word `.docx` files up to 5 MB each are accepted. For a Word file the list also shows a text generated from it for the assistant (由……生成，供助手阅读); the assistant reads that text, and you see the Word file itself. The assistant sees a newly uploaded material from the next session it starts, so upload before you open a session.
