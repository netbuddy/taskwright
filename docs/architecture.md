...
| Judges | model calls made inside tools, with no tools of their own | A reviewer that checks each item against the domain rules is planned. Whether the user accepts an item is not judged by a model: opening the item counts as reading and confirming it. |
...
| `review` | a reviewer's verdict on an item at one revision (written once the reviewer ships). |
