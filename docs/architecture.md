...
| Executor | the agent inside pi, following the task type's skill | Reads materials, writes items with sources, asks about what is unclear, replies through the reply tool. |
| Judges | model calls made inside tools, with no tools of their own | The confirmation reader decides whether the user's words accept an item. A reviewer that checks each item against the domain rules is planned. |
...
3. The user's words go to pi. The executor calls tools; each write becomes a new item version plus one row in the `event` table.
...
6. Generating a document renders the selected item versions through the task type's template. No model is involved.
...
| `item_version` | one immutable version of an item's fields. |
| `item_source` | one place a source supports in one item version: kind (document excerpt, user's words, added by the agent, direct user edit), locator, verbatim excerpt, field and list index. |
| `review` | a reviewer's verdict on an item version (written once the reviewer ships). |
| `judgement` | one confirmation decision: its basis (a click in the interface, or the user's words read by the confirmation reader). |
| `judgement_item` | which item versions one judgement accepted or rejected. |
