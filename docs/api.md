...
| `GET …/materials/content?path=…` | a material's text | `{ok, path, text}`; the path must stay inside the materials directory. For a `.docx` the text is the generated paragraph-numbered text (see **Materials** in 5.1) |
...
**Materials.** `POST …/materials` (multipart, one file): `.md`, `.txt` or Word `.docx`, at most 5 MB, stored in the task's materials directory (a number is appended to duplicate names; names with path separators are rejected). Returns `{ok, path}`. For a `.docx` the service also writes a text for the assistant beside it, `<name>.docx.txt`: one line per paragraph, starting with `[第 N 段]` (paragraph N) or, in a table, `[第 N 段 · 表 t 行 r 列 c]`. Paragraphs are counted in the body of `word/document.xml`, table and nested-table paragraphs included, text-box paragraphs left out; headers, footers, footnotes, endnotes and comments are not counted. The material list returns both files. A `.docx` that cannot be read is refused with `unsupported_type`, and names ending in `.docx.txt` are refused with `bad_request`. When a message's `attachments` include a `.docx`, the text sent to the assistant says to read the `.txt` beside it.
