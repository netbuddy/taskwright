...
class Index:
    ...
    def task_detail(self, key: str) -> dict:
        ...
        return {
            ...
            "评审与交付": {
                "有没有": False,
                "说明": "这一版还没有交付记录；评审记录是修订统一之后才有的，这种旧格式的任务库里没有，"
                        "所以这里不画徽标。",
            },
    ...
    def current_task_detail(self, key: str) -> dict:
        ...
        return {
            ...
            "修订次数": projection["修订次数"],
            "涉及会话": sessions,
            "评审与确认": "评审由用户在界面上发起（也可在对话里要求助手发起），评审者按规则清单逐条评，发现带规则编号与级别，记在 review 与 review_finding 两张表里，看板的「评审」一栏按最新一次记录显示；确认标记（已读、界面修改、撤回）在「用户已确认」一栏里看。",
