...
· r1：用户请求整理（理解第一次没按格式写，记一条无效）；助手告知一条、请确认 UC-001（期待回应）。
· r2：用户纠正 UC-001 的名称（回应 r1-3），又询问一件事；助手提问 TBD-001（期待回应，没有人回应）。
· r3：这次运行在归档里没有（只在库里）：助手又提问 TBD-001，也没有人回应，于是 TBD-001 连着 2 次运行没得到回应。
...
def add_dialogue(workspace: Path, session_id: str = ARCHIVE_SESSION) -> None:
    ...
    try:
        ...
        event("USER_INTENT_INVALID", "intent-u-1", {"run_id": "r1", "user_entry": "u-1", "reason": "这一轮第一段文字里没有用 ```json 围栏包住的理解"})
