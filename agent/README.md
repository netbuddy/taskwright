...
| `tools/get_task_status.ts` | 「查询任务状态」（`get_task_status`）：各集合的条目、完成条件逐项、未解决的待定事项、最近一次修订。只读，输出与 `/tw-board` 同源。 |
| `tools/record_confirmation.ts` | 「登记用户确认」：用户在对话里接受了某几个条目时由执行者调用，工具把用户原话交给确认判读者（一次不带工具的模型调用），结论由代码写库。 |
...
| `lib/reply.ts` | 「回复」的核对：形式核对、提问等主行为必须挂在条目上、请确认只认当前版本、连续被拒的计数与上限。 |
| `lib/task_query.ts` | 「查看条目」与「查询任务状态」的核心逻辑，只读库。 |
| `lib/record_confirmation.ts` | 「登记用户确认」的核心逻辑：核对、装配给确认判读者的提示、解析判读结果、写库。 |
| `lib/complete_task.ts` | 「完成任务」的核心逻辑。 |
| `lib/user_ops.ts` | 用户直接操作的核心逻辑，`/tw-user` 调用它；改到的字段写「用户直接修改」来源，通知正文带改后的值。 |
...
{ "op": "update", "item": "UC-001", "base_version": 2, "fields": { 只写要改的字段 }, "sources": [ 可省略 ] }
{ "op": "delete", "item": "UC-001", "base_version": 2 }
```

修改与删除必须带 `base_version`，即模型所见的改前版本号；它与库里的当前版本不符时整批拒绝，拒绝的话写明
「条目 UC-001 已经被用户改到第 3 版（你看到的是第 2 版），请先读最新内容再改」并附上当前内容，「被谁改」
取自那一版所在事件的发起方。
...
不写或空列表表示支持整个条目；字段必须是这个集合声明的、这一版里不为空的，`index` 只用于列表型字段并且小于项数。
...
- 启动配置关掉了 pi 的技能自动发现（`--no-skills`），免得本机全局目录里的技能混进来；任务目录里
  `.pi/skills/` 下的技能改用 `--skill <任务目录>/.pi/skills` 显式加载。
