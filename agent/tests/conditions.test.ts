...
test("「至少一个条目」：没有条目时不满足，有了就满足，删光又不满足", () => {
  ...
  saveRevision(callIn(dir), { operations: [{ op: "delete", item: "UC-001", base_revision: 1 }] });
  assert.equal(check().satisfied, false);
});

test("「没有状态为未解决的条目」只看每个条目当前所在修订的内容", () => {
  const dir = makeWorkspace();
  createTask(callIn(dir), { definition_path: DEFINITION_PATH });
  const check = () => withDb(dir, (db) => CONDITIONS["没有状态为未解决的条目"](db, "TASK-001", "问题"));
  assert.equal(check().satisfied, true);
  saveRevision(callIn(dir), { operations: [{ op: "add", collection: "问题", fields: { 事项: "问", 状态: "未解决" }, sources: [SOURCE] }] });
  const unmet = check();
  assert.equal(unmet.satisfied, false);
  assert.deepEqual(unmet.unmet.map((u) => u.item), ["TBD-001"]);
  saveRevision(callIn(dir), { operations: [{ op: "update", item: "TBD-001", base_revision: 1, fields: { 状态: "已解决" } }] });
  assert.equal(check().satisfied, true);
});
...
const COMPLETION = { 用例: ["至少一个条目", "每个条目评审通过", "每个条目用户确认"], 问题: ["没有状态为未解决的条目"] };
