...
const sourcesOf = (dir: string, item: string, version: number) =>
  query<any>(dir, "SELECT position, kind, locator, excerpt, field, field_index FROM item_source WHERE item_id = ? AND version_no = ? ORDER BY position, support_no", item, version)
    .map((row) => ({ ...row }));

// ───────────── 查看条目 ─────────────

test("查看条目：当前版本的全部字段、来源、评审与确认；文字里写明改时 base_version 写几，并附字段原值", () => {
  const dir = fixture();
  const outcome = getItem(dir, { item_id: "uc-001" });
  assert.equal(outcome.details.item_id, "UC-001");
  assert.equal(outcome.details.version_no, 1);
  assert.equal(outcome.details.current_version, 1);
  ...
  assert.equal(outcome.details.confirm, "还没有确认记录");
  assert.match(outcome.text, /^条目 UC-001（集合「用例」），第 1 版，是当前版本。/);
  assert.match(outcome.text, /要修改或删除这个条目时，base_version 写 1。/);
  assert.match(outcome.text, /这一版各字段的原值（JSON）：\{"名称":"登录","步骤":\["打开页面","输入口令"\]\}$/);
});

test("查看条目：可以看旧版本；没有的条目、删掉的条目、没有的版本都抛异常说明", () => {
  const dir = fixture();
  saveRevision(callIn(dir), { operations: [{ op: "update", item: "UC-001", base_version: 1, fields: { 名称: "用口令登录" } }] });
  const old = getItem(dir, { item_id: "UC-001", version_no: 1 });
  assert.equal(old.details.version_no, 1);
  assert.equal(old.details.current_version, 2);
  assert.match(old.text, /这是旧版本；当前版本是第 2 版/);
  assert.throws(() => getItem(dir, { item_id: "UC-009" }), /这个任务里没有条目 UC-009。现有的条目是：UC-001、UC-002、TBD-001。/);
  assert.throws(() => getItem(dir, { item_id: "UC-001", version_no: 5 }), /条目 UC-001 没有第 5 版，它有第 1 到第 2 版。/);
  assert.throws(() => getItem(dir, { item_id: "UC-001", version_no: 0 }), /version_no 应当是从 1 起的整数/);
  assert.throws(() => getItem(dir, {}), /item_id 要写条目编号/);
  saveRevision(callIn(dir), { operations: [{ op: "delete", item: "UC-002", base_version: 1 }] });
  assert.throws(() => getItem(dir, { item_id: "UC-002" }), /条目 UC-002 已在第 3 次修订删除/);
  assert.throws(() => getItem(makeWorkspace(), { item_id: "UC-001" }), /还没有任务记录/);
});

// ───────────── 查询任务状态 ─────────────

test("查询任务状态：各集合的条目、完成条件与看板逐行相同、未解决的待定事项、最近一次修订与事件序号", () => {
  const dir = fixture();
  ...
  assert.deepEqual(lines.slice(0, 4), [
    ...
    "",
    "用例 2 个：UC-001（第 1 版）、UC-002（第 1 版）。",
    "待定事项 1 个：TBD-001（第 1 版）。",
  ]);
  ...
  assert.ok(outcome.text.includes("未解决的待定事项 1 条：\n  TBD-001（第 1 版）：口令长度？"));
  assert.ok(outcome.text.includes("最近一次修订是第 1 次（事件序号 2）。"));
  assert.equal(lines[lines.length - 1], board[board.length - 1]);
  assert.deepEqual(outcome.details.items, { 用例: ["UC-001", "UC-002"], 待定事项: ["TBD-001"] });
  assert.equal(outcome.details.last_revision_no, 1);
  assert.equal(outcome.details.last_event_seq, 2);
  assert.deepEqual(outcome.details.unresolved, [{ item_id: "TBD-001", version_no: 1, title: "口令长度？" }]);
...
test("查询任务状态：还没有修订时如实说明", () => {
  ...
  assert.ok(outcome.text.includes("未解决的待定事项：没有。"));
});
...
test("改字段：改到的字段来源换成「用户直接修改」，没改的字段来源沿用，同时支持两处的来源只去掉改到的那一处", () => {
  const dir = fixture();
  const result = op(dir, { kind: "edit_fields", targets: [{ item_id: "UC-001", base_version: 1 }], fields: { 名称: "用口令登录" } });
  assert.equal(result.note, "界面操作（不是用户打的字）：用户把 UC-001 的「名称」改成了第 2 版。改后的内容是：\n「名称」：用口令登录");
...
test("改列表字段：通知逐条列出，摘录取新值前 200 字；支持整个条目的来源保留", () => {
  ...
  const result = op(dir, { kind: "edit_fields", targets: [{ item_id: "UC-002", base_version: 1 }], fields: { 步骤: ["点注销", long] } });
...
test("清空可选字段：通知写明清空了，不给空字段写来源", () => {
  const dir = fixture();
  op(dir, { kind: "edit_fields", targets: [{ item_id: "UC-002", base_version: 1 }], fields: { 备注: "临时" } });
  const result = op(dir, { kind: "edit_fields", targets: [{ item_id: "UC-002", base_version: 2 }], fields: { 备注: "" } });
...
test("标为先不管：状态字段的来源换成「用户直接修改」；删除的通知写明已删除；撤销的通知写出撤销后的内容", () => {
  ...
  assert.match(del.note, /用户删除了 UC-002（删除前是第 1 版）。这些条目已删除，不再算在交付物里。$/);
  ...
  assert.match(undo.note, /改回 UC-001（现在是第 3 版）。\nUC-001 现在的内容是：\n「名称」：登录\n「步骤」：\n  1. 打开页面\n  2. 输入口令$/);
  // 撤销把来源也改回去了。
  assert.deepEqual(sourcesOf(dir, "UC-001", 3), sourcesOf(dir, "UC-001", 1));
});
...
test("执行者修改时给了新来源：没改到的字段上的「用户直接修改」来源自动带上；改到了就不带", () => {
  ...
  saveRevision(callIn(dir), {
    operations: [{ op: "update", item: "UC-001", base_version: 2, fields: { 步骤: ["打开页面", "输入口令", "点登录"] }, sources: [{ ...SOURCE, supports: [{ field: "步骤" }] }] }],
  });
  assert.deepEqual(sourcesOf(dir, "UC-001", 3).map((row) => [row.kind, row.field]), [["用户直接修改", "名称"], ["文档原文", "步骤"]]);
  saveRevision(callIn(dir), {
    operations: [{ op: "update", item: "UC-001", base_version: 3, fields: { 名称: "账号登录" }, sources: [{ ...SOURCE, supports: [] }] }],
  });
  // 这次改的正是「名称」：用户直接修改那条不再沿用；「步骤」上的来源沿用。
  assert.deepEqual(sourcesOf(dir, "UC-001", 4).map((row) => [row.kind, row.field]), [["文档原文", "步骤"], ["文档原文", null]]);
});

test("加入第四种来源之前建的库不认它：改字段照旧能改，来源沿用上一版", () => {
  const dir = fixture();
  ...
  db.close();
  const result = op(dir, { kind: "edit_fields", targets: [{ item_id: "UC-001", base_version: 1 }], fields: { 名称: "用口令登录" } });
  assert.equal(result.results[0].version_no, 2);
