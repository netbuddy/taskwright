...
例外一处：`intent_record.ts` 写库。它写的是对话行为表 `dialogue_act` 与两种事件（`USER_INTENT_RECORDED`、`USER_INTENT_INVALID`），
不碰交付物。理解写在执行者一轮的第一段文字里，不经任何工具，只有助手消息落进会话的事件拿得到它，所以只能在这里记。
