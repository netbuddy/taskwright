...
export function materialsSentence(materials: { dir: string; files: MaterialFile[] }): string {
  ...
  return `材料目录 ${materials.dir} 里有 ${materials.files.length} 个文件：${materials.files.map((f) => `${f.path}（${sizeText(f.bytes)}）`).join("、")}。`;
}

const materialsDetails = (materials: { dir: string; files: MaterialFile[] }) => ({
  dir: materials.dir,
...
function changesSince(
  ...
  sessionId: string,
  materials: { dir: string; files: MaterialFile[] },
