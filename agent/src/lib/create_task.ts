...
export function describeDeliverable(definition: TaskDefinition): string {
  ...
  lines.push(
    ...
    "每个新增的条目至少要带一条来源。每条来源有：kind 是种类，只能是「文档原文」「用户的话」「执行者补充」「领域说明」之一；" +
      "locator 是出处，文档原文写文件路径，执行者补充写「执行者补充」，领域说明写那条领域说明的条目编号（任务里有「领域说明」集合时才用），" +
      "用户的话不写、由工具在对话里找到那句话代填；" +
    ...
    "修改或删除条目时要写 base_revision，也就是你所见的这个条目当前所在的修订号；它与库里的不符时整批拒绝。",
    "修改条目时只写要改的字段；省略 sources 就沿用它当前的来源，给了就只替换改到的字段上的来源。",
