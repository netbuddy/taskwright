    ...
    Type.Object(
      {
        ...
        version_no: Type.Optional(Type.Integer({ description: "条目的当前版本号；只能评当前版本。" })),
      },
      { additionalProperties: true },
    ),
    { description: "要评审的条目，每项是 { item_id, version_no }。不写就评全部还没有评审通过的条目。" },
  )),
});
...
export function registerRequestReview(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    label: "请求评审",
    description:
      "保存条目之后调用，请评审者按领域规矩文档评审条目的当前版本。items 不写就评全部还没有评审通过的条目。" +
      "它会逐条告诉你合规还是不合规，不合规的列出每一处发现（哪个字段、第几项、问题、建议）；按发现改好之后再请求评审。" +
      "同一条目的同一版连续三次不合规就不再评，这时要把三次的发现原样转给用户，问他怎么办。",
